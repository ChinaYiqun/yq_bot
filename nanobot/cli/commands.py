...
def gateway(
    port: int | None = typer.Option(
        None, "--port", "-p", help="Gateway port (defaults to config)"
    ),
    ...
    # Allow CLI to override the gateway/web UI port (if enabled).
    if port is not None:
    ...
    async def on_heartbeat(prompt: str) -> str:
        ...
        return await agent.process_direct(prompt, session_key="cli:heartbeat")
    ...
    if channels.enabled_channels:
        ...
        if "web" in channels.enabled_channels:
            console.print(
...
def channels_status():
    ...
    # Web UI
    web = getattr(config.channels, "web", None)
    if web is not None:
        table.add_row(
            "Web",
            "✓" if web.enabled else "✗",
